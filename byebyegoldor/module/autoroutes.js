...
import DmapDungeon from "../utils/DmapDungeon"
...
const wallClip = register("tick", () => {
    ...
    if (coords.some(tolerance => isWithinTolerance(fx, tolerance)) || coords.some(tolerance => isWithinTolerance(fz, tolerance))) {
        if (config().Autoroute && config().AutorouteDebug) sendMsg(`&dWall Clipped`)
...
let module = "byebyegoldor"
let path = "data/AutoRoutes/Autoroute.json"
let nodes = {}
let editMode = false
let isKeyHeld = false
...
function handleAutoroute(autoroute) {
    ...
    switch (autoroute.type) {
        ...
        case "use":
            if (config().Autoroute && config().AutorouteDebug) sendMsg(`&7Using: &d${autoroute.data.item}&7, &d${yaw}&7, &d${autoroute.data.pitch}`)
            stopWalk()
            setSneakKey(false)
...
function addAutoroute(autoroute) {
    ...
    saveAutoroute()
    if (config().Autoroute && config().AutorouteDebug) sendMsg(`&7Added autoroute: &d${roomName}`)
}
    ...
    if (nearestNode && closestDistance <= 2.5) {
        ...
        if (config().Autoroute && config().AutorouteDebug) {
            if (nearestNode.chain) {
                sendMsg(`&7Chained autoroute: &d${nearestNode.x}&7, &d${nearestNode.y}&7, &d${nearestNode.z} &7in room: &d${roomName.name}`)
            } else {
                sendMsg(`&7Unchained autoroute: &d${nearestNode.x}&7, &d${nearestNode.y}&7, &d${nearestNode.z} &7in room: &d${roomName.name}`)
            }
        }
    } else {
...
function removeAutoroute() {
    ...
    if (nearestNode && closestDistance <= 2.5) {
        ...
        saveAutoroute()
        if (config().Autoroute && config().AutorouteDebug) sendMsg(`&7Removed autoroute: &d${nearestNode.x}&7, &d${nearestNode.y}&7, &d${nearestNode.z} &7in room: &d${roomName.name}`)
    } else {
