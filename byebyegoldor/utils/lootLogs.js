import { getSellPrice } from "../utils/prices"
import { appendToFile, formatNumber, getFormattedNameFromId } from "../utils/ACutils"
...
export const logLoot = (floor, chestInfo, chestCount) => {
    ...
    appendToFile("byebyegoldor", "data/AutoCroesusData/runLoot.txt", final)
}
...
export const handleLootCommand = (...args) => {
    ...
    if (!FileLib.exists("byebyegoldor", "data/AutoCroesusData/runLoot.txt")) {
        ChatLib.chat(`No loot has been logged!`)
        return
    }

    const lines = FileLib.read("byebyegoldor", "data/AutoCroesusData/runLoot.txt").split("\n").reverse()
    let dungeons = 0
    let loot = {}
